...
**`menuVersion` 只記在訂單層級，加點時不會更新。** 兩條寫入路徑（`applyOrderIntent`
與這支）都一樣。所以一張單如果橫跨了一次菜單發佈，文件上的 `menuVersion` 只代表
第一次下單時的版本，對後來加的那幾行來說是錯的。

不在這裡修，是因為把它更新成最新版本對更早的那些行一樣是錯的——準確的紀錄本來就在
每一行的價格快照裡（`unitPriceDineIn`、`unitPriceTakeout`、選項的 `priceDelta`），
`menuVersion` 只是對帳時的方便指標。要真的做到可追溯得把版本記到 line 層級，
那是 `orders` 結構加兩條寫入路徑一起改，等有人真的在對帳時被絆到再說。

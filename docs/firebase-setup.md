...
5. 在「已送出」畫面按「我要加點」再送一次，**不會**變成第二張單，
   ...
   加點的菜單頁最上面有一條「已點 N 份・小計 $X」，按得下去，拉開是已點項目的明細。
6. 換一支手機（或換一個瀏覽器）掃同一張 QR code，菜單頁上會出現
   「本桌已有未結帳訂單：N 份，小計 $X。新增品項將併入同一張訂單。」——
   同桌的第二個人掃進來看到的就是這個。它只有份數與金額、點不開，因為伺服器
   不回別人那張單的品項明細。
7. 把那一攤結掉（照下面〈想再測一次的話〉清乾淨），用**同一支手機、同一個分頁**
   再掃一次：不會再跳出剛才那張已經結掉的單，而是菜單頁加一句
   「先前的訂單已結帳，本次為新訂單。」
